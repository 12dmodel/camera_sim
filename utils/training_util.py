...
class MovingAverage(object):
    ...
    def update(self, val):
        ...
        if len(self._cache) > self.n:
            del self._cache[0]
        self.mean = np.mean(self._cache)

    def get_value(self):
        return self.mean


def save_checkpoint(state, is_best, checkpoint_dir, n_iter):
    filename = os.path.join(checkpoint_dir, "{}.pth.tar".format(n_iter))
